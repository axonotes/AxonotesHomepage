...
>
    ...
    {#if page.status === 404}
        <h1
                class="text-primary-600 dark:text-primary-400 mb-4 text-4xl font-bold !leading-tight md:text-5xl lg:text-6xl"
        >
            Page Not Found
        </h1>
        <p
    ...
    {:else}
        <h1
                class="text-warning-600 dark:text-warning-400 mb-4 text-4xl font-bold !leading-tight md:text-5xl lg:text-6xl"
        >
    ...
    {#if dev && page.error?.message}
        <div
                class="border-surface-200 dark:border-surface-700 mt-12 w-full max-w-2xl rounded-lg border bg-surface-50 p-4 text-left dark:bg-surface-800/50"
        >
            <p
            ...
            <pre
                    class="text-error-600 dark:text-error-400 whitespace-pre-wrap text-xs font-mono"
            ><code>{page.error.message}</code></pre>
        </div>
    {/if}
</div>